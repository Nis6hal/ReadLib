...
import { ArrowLeft, ChevronLeft, ChevronRight, Maximize, Sun, Moon, Coffee } from 'lucide-react';
...
function EpubViewer() {
  ...
  useEffect(() => {
    if (!bookData || !bookData.fileHandle) {
      setError('Book not found or file handle unavailable.');
      setLoading(false);
      return;
    }
    ...
    async function loadEpub() {
      try {
        ...
        rendition.on('relocated', (loc) => {
          if (!cancelled) {
            ...
            updateBook({
              ...
              category: bookData.category === 'Planned' ? 'Reading' : bookData.category,
            });
          }
        });
        ...
        if (!cancelled) {
          setError('Failed to load EPUB. It might be corrupted or in an unsupported format.');
          setLoading(false);
        }
      }
    }
  ...
  const applyTheme = (theme) => {
    ...
    const bgColor = theme === 'night' ? '#0f172a' : theme === 'sepia' ? '#f4ecd8' : '#ffffff';
    ...
    <div className={`epub-viewer-container reader-theme-${readerTheme}`}>
      <div className="epub-toolbar glass-panel">
        <div className="epub-toolbar-left">
          <button className="btn btn-icon" onClick={() => navigate(-1)}><ArrowLeft size={20} /></button>
          <span className="epub-title">{bookData?.title}</span>
        </div>
        
        <div className="epub-toolbar-right">
          <div className="toolbar-group">
            <button className="btn btn-icon" onClick={() => changeFontSize(-10)}>-</button>
            <span className="font-size-label">{fontSize}%</span>
            <button className="btn btn-icon" onClick={() => changeFontSize(10)}>+</button>
          </div>
          <div className="toolbar-divider"></div>
          <div className="toolbar-group">
            <button className={`btn btn-icon ${readerTheme === 'light' ? 'active' : ''}`} onClick={() => applyTheme('light')}><Sun size={18} /></button>
            <button className={`btn btn-icon ${readerTheme === 'sepia' ? 'active' : ''}`} onClick={() => applyTheme('sepia')}><Coffee size={18} /></button>
            <button className={`btn btn-icon ${readerTheme === 'night' ? 'active' : ''}`} onClick={() => applyTheme('night')}><Moon size={18} /></button>
          </div>
          <div className="toolbar-divider"></div>
      ...
      <button className="nav-btn nav-btn-left" onClick={() => renditionRef.current?.prev()} title="Previous Page">
        <ChevronLeft size={32} />
      </button>
      <button className="nav-btn nav-btn-right" onClick={() => renditionRef.current?.next()} title="Next Page">
